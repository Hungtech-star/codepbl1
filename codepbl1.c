...
// Hàm quay lui bổ trợ kiểm tra nhanh sự tồn tại của chu trình Hamilton
int h_chu_trinh_backtrack(int u, int weight[MAX_N][MAX_N], int n, int visited[], int count, int start_vertex) {
    // Nếu đã đi qua tất cả các đỉnh, kiểm tra xem có đường về đỉnh xuất phát không
    if (count == n) {
        if (weight[u][start_vertex] != -1) return 1;
        return 0;
    }

    for (int v = 0; v < n; v++) {
        if (weight[u][v] != -1 && u != v && !visited[v]) {
            visited[v] = 1;
            if (h_chu_trinh_backtrack(v, weight, n, visited, count + 1, start_vertex)) {
                return 1; // Tìm thấy ít nhất 1 chu trình hợp lệ
            }
...
void ktra_mt(int weight[MAX_N][MAX_N], int n, char nameIndex[MAX_N]) {
    ...
    // BƯỚC 1: Kiểm tra hướng của đồ thị
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
        ...
        if (!is_undirected) break;
    }

    // BƯỚC 2: Kiểm tra điều kiện cần cơ bản về bậc
    for (i = 0; i < n; i++) {
        int outDeg = 0, inDeg = 0;
        for (j = 0; j < n; j++) {
            if (weight[i][j] != -1 && i != j) outDeg++;
            if (weight[j][i] != -1 && i != j) inDeg++;
        }

        if (is_undirected) {
            if (outDeg < 2) {
                printf("\n[LOI DIEN KIEN] Dinh %c co bac %d < 2. Khong the co chu trinh Hamilton!\n", nameIndex[i], outDeg);
                exit(1);
            }
        } else {
            if (outDeg < 1 || inDeg < 1) {
                printf("\n[LOI DIEN KIEN] Dinh %c thieu canh vao/ra (out:%d, in:%d). Khong the co chu trinh Hamilton!\n", nameIndex[i], outDeg, inDeg);
                exit(1);
            }
        }
    }

    // BƯỚC 3: Quét cấu trúc thực tế xem có tồn tại chu trình Hamilton nào không
    int visited[MAX_N] = {0};
    int start_vertex = 0; // Chọn đỉnh 0 làm mốc xuất phát thử nghiệm
    visited[start_vertex] = 1;

    // Gọi hàm thử nghiệm tìm chu trình
    if (!h_chu_trinh_backtrack(start_vertex, weight, n, visited, 1, start_vertex)) {
        printf("\n[LOI CAU TRUC] Do thi hop le ve bac nhung phan bo canh co dinh (khop/nghen).\n");
        printf("=> KHONG TON TAI CHU TRINH HAMILTON DI QUA TAT CA CAC DINH!\n");
        printf("Chuong trinh dung lai.\n");
        exit(1);
    }

    printf("\n[OK] Do thi hop le và chac chan co chu trình kha thi. San sang tinh toan...\n");
}
