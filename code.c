...
// ===================== SỬA DUY NHẤT HÀM NÀY =====================
void readInput(FILE *file,
               int weight[__MAX_N][__MAX_N],
               int indexMap[__MAX_CHAR],
               char nameIndex[__MAX_N],
               int *n,
               char *startNode) {
    ...
    *n = 0;
    ...
    while (fscanf(file, " %c %c %d", &u, &v, &w) == 3) {
        ...
        // Luu canh u -> v
        // Neu trung canh thi lay trong so lon nhat
...
void writeResult(const char *filename, struct ResultWay* result,
                 int best, char startNode, const char *nameTypeSolve) {
    ...
    if (best <= 0) {
        ...
        struct ResultWay *tmp = result;

        do {
            fprintf(out, "%c -> ", tmp->name);
            printf("%c -> ", tmp->name);
            tmp = tmp->next;
        } while (tmp->name != startNode);
...
void kTraMTran(int weight[__MAX_N][__MAX_N], int n, char nameIndex[__MAX_N]) {
